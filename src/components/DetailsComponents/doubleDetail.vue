<template>
  <div class="details-page" v-if="!loading">
    ...
    <div class="product-image">
      <img :src="`${double.imageUrl}`" alt="产品图片" />
    </div>
    ...
    <div class="product-details">
      <h3>商品详情</h3>
      <div class="detail-content">{{ double.description }}</div>
    </div>
...
const double = ref(null);

onMounted(async () => {
  try {
    // 获取双重奶详情
    await productStore.getById(id);
    double.value = productStore.product;
    console.log(double.value); // 添加调试信息
    if (!double.value) {
      console.error("未能获取到双重奶数据");
    }
  } catch (error) {
    console.error("获取双重奶详情时出错", error);
  } finally {
    loading.value = false;
  }
});
...
const addCart = () => {
  if (double.value) {
    shopcarStore.addToCart(double.value.productId, quantity.value, 'product');
  }
  alert("加入购物车成功");
  history.back();
};
